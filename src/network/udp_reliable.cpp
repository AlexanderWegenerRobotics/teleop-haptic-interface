// Direct port from avatar project src/network/udp_reliable.cpp
...
void UdpReliable::registerHandler(const std::string& msg_type,
...
void UdpReliable::send(const std::string& msg_type,
                        const msgpack::sbuffer& payload_buf,
                        bool ack_requested) {
    sendImmediate(msg_type, payload_buf, ack_requested,
                  config_.default_retry_ms, config_.max_retries);
}

void UdpReliable::sendImmediate(const std::string& msg_type,
                                 const msgpack::sbuffer& payload_buf,
                                 bool ack_requested,
                                 int  retry_ms,
                                 int  max_retries) {
...
msgpack::sbuffer UdpReliable::packEnvelope(const std::string& msg_type,
                                            const msgpack::sbuffer& payload_buf,
                                            bool ack_requested) {
    std::lock_guard<std::mutex> lock(send_mtx_);
    ++send_seq_;
    ...
    msgpack::object_handle oh =
        msgpack::unpack(payload_buf.data(), payload_buf.size());
        ...
        try {
            ...
            if (env.msg_type == "ack") {
                ...
                if (env.ack_requested) {
                    ...
                    transport_.sendTo(packed.data(),
                                      static_cast<int>(packed.size()), sender);
                }
                dispatchMessage(env);
            }
...
bool UdpReliable::isAlive() const {
    auto now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::steady_clock::now().time_since_epoch()).count();
    return (static_cast<uint64_t>(now_ms) - last_recv_time_ms_.load())
           < ALIVE_TIMEOUT_MS;
}
