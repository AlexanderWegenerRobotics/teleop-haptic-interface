...
// Direct port from avatar project src/network/udp_stream.hpp
// Keep in sync with the avatar version.
...
// UdpStream<TRecv, TSend>
// Runs a background thread that:
//   - receives TRecv packets (newest-wins, drops out-of-order)
//   - sends    TSend packets at send_rate_hz
//
// Both message types MUST:
//   1. Be trivially copyable
//   2. Start with MsgHeader as their first member
template<typename TRecv, typename TSend>
class UdpStream {
    static_assert(std::is_trivially_copyable_v<TRecv>,
        "TRecv must be trivially copyable");
    static_assert(std::is_trivially_copyable_v<TSend>,
        "TSend must be trivially copyable");
    static_assert(offsetof(TRecv, header) == 0,
        "TRecv must start with MsgHeader (header field at offset 0)");
    static_assert(offsetof(TSend, header) == 0,
        "TSend must start with MsgHeader (header field at offset 0)");
...
public:
    ...
    // Thread-safe: set the outgoing message payload for the next send cycle
    void setSendData(const TSend& msg) {
        std::lock_guard<std::mutex> lock(send_mtx_);
        send_msg_ = msg;
    }

    // Thread-safe: retrieve the most recently received message (clears hasNew flag)
    TRecv getRecvData() {
    ...
    // Stamp the outgoing header with a system state / fault
...
private:
    ...
    void receive() {
        ...
        while (true) {
            ...
            if (n == static_cast<int>(sizeof(TRecv))) {
                TRecv msg;
