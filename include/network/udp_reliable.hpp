...
// Direct port from avatar project src/network/udp_reliable.hpp
...
using MessageHandler =
    std::function<void(const ReliableEnvelope&, const msgpack::object&)>;

// ─── UdpReliable ─────────────────────────────────────────────────────────────
// msgpack-based reliable channel with optional per-message ACK/retry.
// Used for infrequent but important control messages (state transitions, etc.).
class UdpReliable {
public:
    ...
    // Fire-and-forget (queues for retry if ack_requested=true)
    void send(const std::string& msg_type,
              const msgpack::sbuffer& payload_buf,
              bool ack_requested = false);

    // Send immediately with explicit retry params
    void sendImmediate(const std::string& msg_type,
                       const msgpack::sbuffer& payload_buf,
                       bool ack_requested  = false,
                       int  retry_ms       = -1,
                       int  max_retries    = -1);
...
private:
    ...
    msgpack::sbuffer packEnvelope(const std::string& msg_type,
                                  const msgpack::sbuffer& payload_buf,
                                  bool ack_requested);
